...
import { getLocalLlmStatus, startLocalLlmSetup, LocalLlmStatus } from '../services/localLlmService';
...
export default function LocalLlmSetup({ onReady }: Props) {
  ...
  const handleSetup = async () => {
    try {
      await startLocalLlmSetup();
    } catch {
      // error will show via polling
    }
  };

  if (!status) return <div className="local-llm-setup">Checking local LLM status...</div>;

  if (status.modelReady) return null;

  const phase = status.setupPhase;
  const label = phase === 'pulling' && status.pullProgress != null
    ? `${phaseLabels.pulling} (${status.pullProgress}%)...`
    : phaseLabels[phase] || phase;

  const isWorking = phase === 'installing' || phase === 'starting' || phase === 'pulling';
  const isError = phase === 'error';
  const isCrashed = !isWorking && !isError && !status.modelReady && (phase === 'ready' || phase === 'idle');

  return (
    <div className="local-llm-setup" style={{ marginTop: 12, padding: 12, border: '1px solid #444', borderRadius: 6 }}>
      <div style={{ marginBottom: 8, fontWeight: 'bold' }}>Local LLM Setup</div>
